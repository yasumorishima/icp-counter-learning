...
| | |
|---|---|
| **けいさん 36 単元** | 全学年 6 単元ずつ。たし算・ひき算・九九（段ごと）・時計・わり算・分数・小数・がい数・面積・角度・割合・平均・体積・比・速さ・円の面積・場合の数 など |
| **文を 読んで もらえる** | 文章題には「♪ よんで もらう」が 出る。読むのは 端末の 声なので、**声を 持たない 端末では ボタンを 出さない**（押せない ボタンを 見せない）。文章題で つまずく 理由は 計算より **文が 読めない** ことの ほうが 多い |
| **まちがえたら しきを 見せる** | 文章題を まちがえた ときは、答えの 数字だけで なく **どう とくか**（しき・きまり・あまりの 出しかた）を その場で 出す。正解した ときは 出さない |
| **ぶんしょうだい 31 単元** | さんすうの もう 1 つの カテゴリー。文を 読んで 式を 立てる。1年 5・2年 6・3〜6年 5 ずつ。**きまりを みつける**（5, 10, 15 … 木よう日は？）・あわせて いくつ・のこりは いくつ・どちらが おおい・おかね・あまりと はこ・なんばい・わりあい・へいきん・速さ・比・わりびき など。名まえも ものも 数も 毎回 変わる |
| **きょうの 1まい** | その日ごとに 決まった 10 問。同じ日なら 何度ひらいても 同じ、日が変われば 変わる |
| **タイムアタック** | 60 びょうで 何問とけるか。学年ごとに さいこう記録 |
| **チャレンジ** | なつやすみ や 30日。毎日 1まい やって、さいごに **印刷できる しょうじょう**。なつやすみは **端末の 場所で 期間が 変わる**（日本 7/21〜8/31・アメリカ 6/10〜8/25）＝日本語の 画面の まま アメリカに 居ても アメリカの 休みが 出る。**誘いは 始まる 3 日前から、終わる 日を 過ぎたら 出さない** |
...
| **あそび** | 幼児向けの 6 つ。もぐら・ふうせん・ことば・さかな・おえかき・おと。**文字が 読めなくても 最後まで できる** |
| **1 画面で えらべる** | トップの 4 つ（さんすう・しょうぎ・そら・あそび）が **スマホの 最初の 画面に 2 x 2 で ぜんぶ 入る**。ドリルも **単元の 一覧が 先**で、きょうの 1まい・タイムアタック・チャレンジは その あとに 線で 区切って 置く |
| **きせつ** | 端末の 時計の 月で 背景が 変わる（3〜5 はる / 6〜8 なつ / 9〜11 あき / 12〜2 ふゆ）。空の 色と、舞う もの（さくらの 花びら・しゃぼん玉・もみじ・ゆき）と、トップの 景色（さくらの木と ちょうちょ・入道雲と ヨット・もみじと すすき・ゆきだるま）。**夜は お日さまが お月さまに なる**。絵は すべて 自前の SVG で、外から 読み込む ものは 無い。「動きを 減らす」設定の 端末では 舞わせず、色だけ きせつの ままに する |
| **よみあげ** | 目が 見えなくても つかえる。そらは 見て いる 空を ことばで 出し、あそびは 読み上げと キーボードでも 遊べる |
| **拡大しない** | こどもが 遊んで いる さいちゅうに 指 2 本で 拡大して しまわない（そら と ささえる ページ だけ 拡大できる） |
| **ことば** | 英語と 日本語。どの画面からでも 切りかえられ、えらんだ ことばは 端末に のこる。星の 名前も 星座の 名前も ことばに あわせて 変わる |
| **日記アプリへの 導線** | トップの 使いかたの 下に 1 行。同じ 作り手の 日記アプリ（[Google Play](https://play.google.com/store/apps/details?id=com.diary.daily)・**Android のみ**）へ 別タブで 開く |
...
3 さいくらいの 子が、ひとりで さわって 遊べる ところ。上の 3 つは どれも 文字が 読める ことを
前提に して いるので、その 前の 段に 入り口を 作った。

| | |
|---|---|
| **もぐら** | 穴から 出た ときに さわる。10 ぴき |
| **ふうせん** | ゆっくり 上がって くる ふうせんを さわって 割る。10 こ |
| **ことば** | 絵を さわると その ことばを **声で 読む**（端末の 読み上げ。無くても 音は 鳴る）。絵は 14 種類。8 回 |
| **さかな** | 水そうを 泳ぐ さかなを さわって すくう。逃しても 責めない。8 ひき |
| **おえかき** | ゆびで じかに 描く。色 6 つと「けす」だけ。終わりも 点も 無い |
| **おと** | 6 つの 音板。さわると その 高さの 音が 鳴る。終わりも 点も 無い |

決めごと:

- **さわるだけ**（おえかき だけ なぞる）。ねらいを 定める ドラッグは 3 さいには 難しい
- **まちがいを 出さない**。×も 減点も 時間切れも 無い。外した ときは 何も 起きない
  （正誤を 問う あそびは 置かない。3 さいには 手応えが 薄い）
- 押す ところは **44px 以上**（検査で 守る）
- 絵は **すべて 自前で 描く**。絵文字は 端末に よって 出ない
  （2026-09-20 に 画面へ 出して いた 絵文字 23 文字を 1 文字ずつ 描いて 調べたところ
  **11 文字が 豆腐**＝えらべる かお 8 つの うち 7 つが 出て いなかった）。
  ★ ♪ ✦ ✕ × □ は ふつうの 記号で どこでも 出るので そのまま 使う
- 記録は **残さない**。キャニスターにも localStorage にも 書かない
- 画面を 離れたら タイマーと 描画を **かならず 止める**（`stopAsobi`。電池の ため）
- **読み上げと キーボードでも 遊べる**（下の「目が 見えなくても つかえるように」）

## 目が 見えなくても つかえるように

画面の 絵を 見なくても 中身が 分かるように しています。読み上げ（スクリーンリーダー）と
キーボードだけで 最後まで 進めます。

### そら — 見て いる 空を ことばで 出す

星空は canvas に 描いた 絵なので、そのままでは 読み上げに 何も 渡りません。
星の 名前・方角・高さ・明るさは 手元で 計算して いるので、それを ことばにして 出します。

```
南の 空（171 度）・高さ 42 度・見える 広さ 105 度 ・ まんなかに ちかいのは ベガ
Looking S (171 degrees), 42 degrees up, 105 degrees wide · Nearest the middle: Vega
```

- 矢印キーで 見まわし、`＋` `−` で ちかづいたり はなれたり、**エンターで まんなかの ものを 読む**
- 方角は ことば（南南東）だけだと 22.5 度ごとに しか 変わらず、矢印 1 回では 同じ ままなので、
  **度も いっしょに 言います**（動いた ことが 耳で 分かる ように）
- 固有名の 無い 星は「星」としか 言えないので **明るさを そえます**（「星 ・ 5.1 等」）

### あそび — 読み上げと キーボードでも 押せる

もぐら・ふうせん・ことば・さかな・おと は もとから `<button>` でしたが、
受けて いたのが `pointerdown` だけでした。**読み上げの 2 回たたきも キーボードの
エンターも `click` しか 出さない**ので、押しても 何も 起きない ままでした。

- `onTap()` で `pointerdown` と `click` の 両方を 受けます。
  指の `click` は `detail >= 1`、機械が 起こした `click` は `detail === 0` なので、
  **0 の ときだけ 通せば 指の タップを 二重に 数えません**
- もぐらは 出入りの たびに 名前が 変わります（「もぐら」／「あな」）
- もぐらと ふうせんは **出た ときに 小さく 鳴らします**（見えなくても 出た ことが 分かる。
  音は 上の 音符ボタンで 切れます）
- 1 つ 1 つの ボタンには **その もの ひとつぶんの 名前**を 付けます。
  音板は「おと 1」〜「おと 6」、おえかきの 色は「みずいろ」「むらさき」…
  （色の 番号 `#38bdf8` が そのまま 読まれない ように）

### そのほか

- しょうぎの 盤は 81 マスが それぞれ ボタンで、「５五 先手歩」のように 名乗ります
- 答えあわせ・とちゅう経過・そらの 読み上げは `role="status"` で 出るたびに 読まれます
- 飾りだけの 絵は `aria-hidden` で 読み上げから 外して います
- **英語と 日本語で 同じ キーが そろって いるかを CI が 毎回 検算します**。
  片方に 無い ことばは 画面では 気づきにくくても、読み上げには そのまま 流れるためです

耳が 聞こえない 場合に ついては、**音は すべて 文字か 光の 裏づけが あります**
（答えあわせは 文字でも 出る／ことばは 単語も 出す／音板は 押すと 光る）。
音そのものが 中身の「おと」だけは、音を 消すと 遊びに なりません。

## 大事にしていること

- **こどものデータを 外に出さない。** 問題は 端末の中で作り、記録も `localStorage` だけ。
  ドリルの 記録・なまえ・こたえを サーバーに 送る ことは ありません
- **外の部品を 読み込まない。** フォント・音・かみふぶきまで 自前。外部の CDN は ゼロ
- **アクセス解析は Google アナリティクス（GA4）だけ。** どの 画面が どれくらい 使われて いるかを 見るため
  （`src/todo_app_frontend/src/analytics.js`）。**本番の ホストでだけ** 読み込み、CI・ローカルでは 何も 送らない
  （E2E が 1 本も 出ない ことを 確かめる）。送るのは 開いた 画面（`#/sky` → `/sky`）と GA4 が 通常 集める もの
  （Cookie `_ga` と `_ga_<ID>`・滞在時間・おおよその 地域・端末の 種類）。Google シグナルと 広告の パーソナライズは 切り、
  Cookie は この ホストだけに 置く。**支援ページの ボタンで この 端末から 止められ**、止めると この 2 つの Cookie も 消す。
  本番で 画面 1 つに page_view が 1 本だけ 飛ぶ こと・止めると 1 本も 飛ばない ことを 実ブラウザで 確かめた（2026-09-26）
- **電波が無くても つかえる。** いちど ひらけば、そのあとは オフラインで 解けます
- **目が 見えなくても つかえる。** 絵しか 無い ところ（星空）は ことばでも 出し、
  さわる ところは 読み上げと キーボードでも 押せます。下に 詳しく 書きました
- **こどもの 指で 勝手に 拡大しない。** 画面を つかうのは こどもで、遊んで いる さいちゅうに
  指が 2 本 当たると 気づかないまま 拡大され、そこから 戻せなく なります。
  そら（星が 小さい）と ささえる ページ（おとなが 読む）だけ 拡大を のこし、
  ほかは meta・touch-action・つまむ 動きの うちけし の 3 つで 止めます
  （1 つでは 端末に よって 止まらない ため）
...
なつやすみの 誘いが 出る 日と 出ない 日（端末の 時計と 時間帯を 動かして、**ボタンの 文字に 出て いる 月日**の
3 日前・4 日前・最終日・その 翌日を 見る。日本語の 画面の まま アメリカの 時間帯に すると
アメリカの 休みに 変わる ことも）、
...
ぶんしょうだい（2 つの カテゴリーが 混ざらない こと・6 学年ぶん ある こと・えらんだ ほうが
読み込み直しでも のこる こと・文が 読む ための 大きさで 左ぞろえに なる こと・
**文に 出た 数から 検査が じぶんで 解いた 答え**で 10 問 とも 正解に なる こと・記録に のこる こと・
まちがえた ときだけ しきが 出て、その しきが 文の 数と 合って いる こと・
文章題だけに 読み上げの ボタンが 出て、端末に わたす 文・ことば・速さが 合って いる こと）、
指 2 本で 拡大しない こと（meta の user-scalable・touch-action・つまむ 動きの うちけし の 3 つを
別々に 見て、**そらだけは 拡大できる ままで ある**ことも 見る）、
...
音そのもの（おとの 6 音の 高さ・長さ・大きさ、ことばの 音と 読み上げに 頼む ことば・声・速さ、音を 切ると 鳴らない こと）と、
声を 持たない 端末で ことばの 画面に「声が ない」と 出る こと（遅れて 届く 声・知らせの 来ない 端末も 含めて 10 とおり、明暗の 色と 320px）まで、
きせつ（時計を 4 つの 日に 置いて、名前・色・舞う もの・景色・夜の お月さま・
上ぶちの 色・並びが 変わらない こと・動きを 減らす 設定・
**きせつ x 明暗 8 とおりで 文字が 読めること**）まで、
トップが **スマホの 1 画面に 4 つとも 入ること**（2 x 2・ボタンの 高さが そろう・
44px 以上・320〜1280px で 横に こぼれない）まで、
ドリルは **単元の 一覧が 3 枚より 先で、1 画面目から 始まる**ことまで、
**画面に 絵文字を 出して いない**こと（かおは 自前の SVG・絵文字で 保存して あった
古い なまえも そのまま 開ける）まで、
**行あたまに 来ては いけない 字**（小書きの かな・長音・句読点・とじかっこ）が
320px と 390px で 1 つも 無いことまで、
**353 項目**を 実ブラウザで 確かめます。

画面が なくても できる 検算（node）は **197 項目 ＋ ことばの そろい ＋ 絵文字の 掃引**です。
しょうぎの きまり 25／あいて 11／終わりかた 39、そらの 計算 29／星の 名前 93、
**英語と 日本語で 同じ キーが そろって いるか**（556 キー・`tests/i18n-keys.test.mjs`）、
**出荷する ファイルに 画面へ 出す 絵文字が 無いか**（`tests/no-emoji.test.mjs`）。

🔴 **絵文字の 掃引は 実ブラウザの 検査とは 別に 要ります**。HTML に 絵文字を
書き戻しても JS が すぐ SVG で 上書きする ので、DOM を 見る 検査には 映りません
（変異で 実測）。ソースを 直接 見る この 検算が それを 捕まえます。
